...
function drawFigureSVG(type, w = 120, h = 120) {
  ...
  switch (type) {
    ...
    case 'triangle': {
      // Diferenciado do equilátero por ser mais baixo e largo
      const triH = H * 0.7; 
      const yBase = cy + triH/2;
      const yTopo = cy - triH/2;
      poly([[cx, yTopo], [cx + W/2, yBase], [cx - W/2, yBase]]);
      break;
    }
    ...
    case 'doritos': {
      ...
      const triW = W * 0.9;
      const el = document.createElementNS(ns, 'polygon');
      el.setAttribute('points', `${cx},${yTopo} ${cx + triW/2},${yBase} ${cx - triW/2},${yBase}`);
      el.setAttribute('fill', '#E8601C');
      el.setAttribute('stroke', '#B8430E');
      el.setAttribute('stroke-width', '2.5');
      el.setAttribute('stroke-linejoin', 'round');
      svg.appendChild(el);
      // Inner texture lines
      const line1 = document.createElementNS(ns, 'line');
      line1.setAttribute('x1', cx - triW*0.22); line1.setAttribute('y1', yTopo + triH*0.38);
      line1.setAttribute('x2', cx + triW*0.22); line1.setAttribute('y2', yTopo + triH*0.38);
      line1.setAttribute('stroke', '#B8430E'); line1.setAttribute('stroke-width', '1.2'); line1.setAttribute('opacity', '0.5');
      svg.appendChild(line1);
      const line2 = document.createElementNS(ns, 'line');
      line2.setAttribute('x1', cx - triW*0.38); line2.setAttribute('y1', yTopo + triH*0.65);
      line2.setAttribute('x2', cx + triW*0.38); line2.setAttribute('y2', yTopo + triH*0.65);
      line2.setAttribute('stroke', '#B8430E'); line2.setAttribute('stroke-width', '1.2'); line2.setAttribute('opacity', '0.5');
      svg.appendChild(line2);
      // Salt dots
      [[cx, yTopo+triH*0.52],[cx-triW*0.15,yTopo+triH*0.72],[cx+triW*0.15,yTopo+triH*0.72]].forEach(([dx,dy]) => {
        const dot = document.createElementNS(ns, 'circle');
        dot.setAttribute('cx', dx); dot.setAttribute('cy', dy); dot.setAttribute('r', '2.5');
        dot.setAttribute('fill', '#FFD580'); dot.setAttribute('opacity', '0.8');
        svg.appendChild(dot);
      });
      break;
    }
...
if (typeof LESSON_CONFIG !== 'undefined') {
  ...
  function updateProgressBar() {
    const pct = totalOriginalQuestions > 0
      ? (correctlyAnswered.size / totalOriginalQuestions) * 100
      : 0;
  ...
  // feedback
  // XP real retornado pelo servidor (considera revisao)
  let lastXpAwarded = 0;
  let lastIsRevision = false;
  ...
  function showFeedback(correct, q) {
    ...
    // Update streak chip in header
    const streakChip = document.getElementById('streak-chip');
    if (streakChip) {
      streakChip.textContent = '🔥 ' + currentStreak;
      streakChip.style.display = currentStreak >= 1 ? 'flex' : 'none';
      streakChip.classList.toggle('streak-hot', currentStreak >= 3);
    }
    ...
    const streakBadge = (correct && currentStreak >= 2)
      ? ` <span class="fb-streak-badge">🔥 ${currentStreak} seguidas!</span>`
      : '';

    const revBadge = lastIsRevision
      ? ' <span style="font-size:.75rem;opacity:.75;background:rgba(255,255,255,.2);padding:2px 7px;border-radius:99px;margin-left:.4rem;">Revisão · ½ XP</span>'
      : '';
    document.getElementById('fb-title').innerHTML =
      (correct ? 'Correto!' : 'Ops! Não foi dessa vez.') + (correct && lastIsRevision ? revBadge : '') + streakBadge;
    document.getElementById('fb-exp').textContent = q.explanation || '';

    if (correct) {
      const awarded = lastXpAwarded || (q.xp || 10);
      xpEarned += awarded;
      answeredCorrect++;
      // Mark question as correctly answered and update progress
      correctlyAnswered.add(q.id);
      updateProgressBar();
      const xpEl = xpChip;
      if (xpEl) {
        const parts = xpEl.textContent.split(' ');
        const cur = parseInt(parts[1]) || 0;
        xpEl.textContent = `⚡ ${cur + awarded} XP`;
  ...
  window.nextQuestion = function() {
    feedbackBanner.style.display = 'none';
    // If last answer was wrong, push question back to end for retry
    if (!lastWasCorrect) {
      questions.push({ ...questions[currentIdx] });
    }
    currentIdx++;
    if (correctlyAnswered.size >= totalOriginalQuestions) {
  ...
  async function submitAnswer(q, correct) {
    try {
      ...
      lastXpAwarded  = data.xp_awarded  || 0;
      lastIsRevision = data.is_revision || false;
    } catch (e) {
      lastXpAwarded  = correct ? (q.xp || 10) : 0;
      lastIsRevision = false;
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btn && side && overlay) {
        ...
        btn.onclick = function() {
            ...
            btn.style.display = 'none';      // Esconde o botão ao abrir
        };

        // Função para fechar (clicando no overlay)
        overlay.onclick = function() {
            side.classList.remove('active');
            overlay.style.display = 'none';
            btn.style.display = 'flex';      // Mostra o botão de volta
        };
